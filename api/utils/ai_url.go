...
// ErrAIDisallowedDestination is returned by the dial guard so the classifier can
// recognise a blocked destination without matching on message text.
...
// AIEndpointURL joins a base URL with an endpoint path, accepting a base that
...
func AIEndpointURL(baseUrl, path string) string {
	...
	// People paste the completions url as their base url, which works for
	// completions and would otherwise send every other call below it.
