...
// AICredentials is one caller's provider for a single request. The endpoint is
// always theirs: the server holds no key of its own.
...
// ResolveAICredentialsForListing resolves a provider for calls that happen before
// a model is known, such as listing the models the endpoint offers.
