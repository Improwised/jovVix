...
// Feeds the prompt's avoid list. A lookup failure degrades to no list.
...
func (ctrl *AIController) TestConnection(c *fiber.Ctx) error {
	...
	ctx, cancel := context.WithTimeout(c.UserContext(), constants.AITestGenerateTimeout*time.Second)
	defer cancel()

	elapsed, sample, err := ctrl.aiSvc.TestConnection(ctx, cred, ctrl.resolveQuestionOptions())
	if err != nil {
...
func (ctrl *AIController) ListModels(c *fiber.Ctx) error {
	...
	listing, err := ctrl.aiSvc.ListModels(ctx, cred)
	if err != nil {
		return utils.JSONFail(c, http.StatusBadGateway, err.Error())
	}

	return utils.JSONSuccess(c, http.StatusOK, listing)
}
...
// Serves both entry points: a quiz id adds that quiz's questions as an avoid list.
